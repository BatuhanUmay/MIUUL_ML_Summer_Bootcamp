...
cart_model = DecisionTreeClassifier(random_state=17).fit(X, y)  # cross validate fit i görmezden gelir kendisi yapar
...
cart_best_grid = GridSearchCV(cart_model, cart_params, cv=5,  # scoring="roc_auc",
...
train_score, test_score = validation_curve(cart_final, X, y, param_name="max_depth", param_range=range(1, 11),
...
joblib.dump(cart_final, 'machine_learning/cart_final.pkl')
cart_model_from_disc = joblib.load('machine_learning/cart_final.pkl')

x = [12, 13, 20, 23, 4, 55, 12, 7]
cart_model_from_disc.predict(pd.DataFrame(x).T)
# cart_model_from_disc.predict([x])
